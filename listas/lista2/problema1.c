...
int main() { // Função principal do código
    ...
    if (imc < 18.5) { // Condicional para verificar se o IMC está abaixo de 18.5
        printf("Sua classificacao com base no IMC: Abaixo do peso");
    } else if (imc >= 18.5 && imc < 25) { // Condicional para verificar se o IMC está entre 18.5 e 25
        printf("Sua classificacao com base no IMC: Peso normal"); 
    } else if (imc >= 25 && imc < 30) { // Condicional para verificar se o IMC está entre 25 e 30
        printf("Sua classificacao com base no IMC: Sobrepeso");
    } else { // Caso o IMC não se enquadre nas condições acima quer dizer que ele está acima de 30, logo o bloco de código abaixo que roda
        printf("Sua classificacao com base no IMC: Obesidade");
    }
